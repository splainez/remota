...
describe("FileListHeader", () => {
	it("renders name, size, and modified headers", () => {
		render(<FileListHeader onSort={vi.fn()} sortKey="name" sortDir="asc" />);
	...
	it("shows descending indicator on active sort column", () => {
		render(<FileListHeader onSort={vi.fn()} sortKey="name" sortDir="desc" />);
	...
	it("calls onSort with 'name' when name header is clicked", async () => {
		...
		const onSort = vi.fn();
		render(<FileListHeader onSort={onSort} sortKey="size" sortDir="asc" />);
	...
	it("calls onSort with 'size' when size header is clicked", async () => {
		const user = userEvent.setup();
		const onSort = vi.fn();
		render(<FileListHeader onSort={onSort} sortKey="name" sortDir="asc" />);
