...
describe("ActiveTransfers", () => {
	it("returns null when not visible", () => {
		const { container } = render(<ActiveTransfers visible={false} onToggle={vi.fn()} />);
		expect(container.innerHTML).toBe("");
	});

	it("renders transfer header and items when visible", () => {
		render(<ActiveTransfers visible={true} onToggle={vi.fn()} />);
	...
	it("calls onToggle when close button is clicked", async () => {
		...
		render(<ActiveTransfers visible={true} onToggle={onToggle} />);
