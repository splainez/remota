...
describe("useFileSelection", () => {
	...
	it("selects a single entry with plain click", () => {
		...
		act(() => {
			result.current.handleSelectEntry("b.txt", false, false, sortedNames);
		});
	...
	it("adds to selection with ctrl+click", () => {
		const { result } = renderHook(() => useFileSelection());
		act(() => {
			result.current.handleSelectEntry("a.txt", false, false, sortedNames);
		});
		act(() => {
			result.current.handleSelectEntry("c.txt", true, false, sortedNames);
		});
	...
	it("removes from selection with ctrl+click on selected", () => {
		...
		act(() => {
			result.current.handleSelectEntry("b.txt", true, false, sortedNames);
		});
		act(() => {
			result.current.handleSelectEntry("a.txt", true, false, sortedNames);
		});
		expect(result.current.selectedNames).toEqual(["b.txt"]);
	});
	...
	it("selects range with shift+click", () => {
		...
		act(() => {
			result.current.handleSelectEntry("d.txt", false, true, sortedNames);
		});
	...
	it("replaces selection on plain click", () => {
		...
		act(() => {
			result.current.handleSelectEntry("e.txt", false, false, sortedNames);
		});
